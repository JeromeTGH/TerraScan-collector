...
func LoadCommunityPoolContent(channelForCommunityPoolContent chan<- lcd.StructReponseCommunityPoolContent, channelForErrors chan<- string) {

	var idxTentatives int

	for idxTentatives = 1 ; idxTentatives <= config.AppConfig.Lcd.NbOfAttemptsIfFailure ; idxTentatives++ {
		communityPoolContent, errCommunityPoolContent := lcd.GetCommunityPoolContent(channelForErrors)
		if errCommunityPoolContent == "" {
			if idxTentatives > 1 {
				...
				mailsender.Sendmail("[TerraScan-collector] " + strconv.Itoa(idxTentatives) + " attempts to get data from LCD successfully", "<p><strong>LoadCommunityPoolContent</strong></p><p>Script did " + strconv.Itoa(idxTentatives) + " attempts to get data from LCD successfully</p>", channelForErrors)
			} else {
				channelForErrors <- "[dataloader] LoadCommunityPoolContent : success"
			}
