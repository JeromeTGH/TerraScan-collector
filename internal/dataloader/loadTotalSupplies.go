...
func LoadTotalSupplies(channelForTotalSupplies chan<- lcd.StructReponseTotalSupplies, channelForLogsMsgs chan<- string) {

	var idxTentatives int

	for idxTentatives = 1 ; idxTentatives <= config.AppConfig.Lcd.NbOfAttemptsIfFailure ; idxTentatives++ {
		totalSupplies, errGetTotalSupplies := lcd.GetTotalSupplies(channelForLogsMsgs)
		if errGetTotalSupplies == "" {
			if idxTentatives > 1 {
				...
				mailsender.Sendmail("[TerraScan-collector] " + strconv.Itoa(idxTentatives) + " attempts to get data from LCD successfully", "<p><strong>LoadTotalSupplies</strong></p><p>Script did " + strconv.Itoa(idxTentatives) + " attempts to get data from LCD successfully</p>", channelForLogsMsgs)
			} else {
				channelForLogsMsgs <- "[dataloader] LoadTotalSupplies : success"
			}
