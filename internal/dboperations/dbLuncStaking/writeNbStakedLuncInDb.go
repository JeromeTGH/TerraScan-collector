...
func WriteNbStakedLuncInDb(dataFromLcd lcd.StructReponseNbStakedLunc, stakingPercentage float64, channelForLogsMsgs chan<- string) {
	...
	if errInsert != nil {
		...
		if strings.Contains(errInsert.Error(), "doesn't exist") {
			...
			mailsender.Sendmail("[TerraScan-collector] failed to insert data in DB", "<p><strong>WriteNbStakedLuncInDb : failed to insert data in DB, on first try</strong></p><p>Error : " +  errInsert.Error() + "</p>", channelForLogsMsgs)
			return
		}
	}
	...
	if bCreateTableNeeded {
		...
		errCreation := CreateNbStakedLuncTable(channelForLogsMsgs)
		if errCreation != nil {
			...
			mailsender.Sendmail("[TerraScan-collector] failed to create table in DB", "<p><strong>WriteNbStakedLuncInDb : failed to create table in DB</strong></p><p>Error : " +  errCreation.Error() + "</p>", channelForLogsMsgs)
			return
		}
		...
		if errInsert2 != nil {
			...
			mailsender.Sendmail("[TerraScan-collector] failed to insert data in DB", "<p><strong>WriteNbStakedLuncInDb : failed to insert data in DB, on second try</strong></p><p>Error : " +  errInsert2.Error() + "</p>", channelForLogsMsgs)
			return
		}
